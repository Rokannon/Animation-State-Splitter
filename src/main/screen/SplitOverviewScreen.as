package main.screen {
	...
	public class SplitOverviewScreen extends PanelScreen {
		...
		private function copyNext():void {
			...
			_progressBar.value = (_currentFileIndex * splitData.stateDatas.length + _currentStateDataIndex) / 
				(files.length * splitData.stateDatas.length);
			...
			if (stateData.startFrame <= _currentFileIndex && _currentFileIndex <= stateData.endFrame) {
				var statePrefix:String = (splitData.addStatePrefix ? "state_" + intToString(_currentStateDataIndex + 1, 2) + "_" : "");
